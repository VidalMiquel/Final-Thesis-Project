...
def getNodeAttributes(dataFrame):
    ...
    for index, row in uniquePlayers.iterrows():
        ...
        playerId = int(float(row["player_id"]))  # Convert to float first, then to int
        playerName = row["player_name"]
        # Add the attribute for the current player to the dictionary
        attributes[playerId] = {"label": playerName}
    return attributes
...
# Define edged between nodes.
def directedEdges(dataframe, G):
    uniquePlayers = (
        dataframe[["player_id", "pass_recipient_id"]].drop_duplicates().dropna()
    )
    ...
    uniquePlayers["pass_recipient_id"] = (
        uniquePlayers["pass_recipient_id"].astype(float).astype(int)
    )

    # Group by 'player_id' and 'pass_recipient_id', and count occurrences
    edgeWeights = (
        uniquePlayers.groupby(["player_id", "pass_recipient_id"])
        .size()
        .reset_index(name="weight")
    )

    # Ensure the weight is integer
    edgeWeights["weight"] = edgeWeights["weight"].astype(int)

    # Add edges to the graph with their weights
    # Convert DataFrame to list of tuples (node1, node2, weight)
    edgeList = edgeWeights.to_records(index=False)
    G.add_weighted_edges_from(edgeList)


# Add atributes to edges
def addEdgesWithAttributes(row, G):
...
def changeFilenames(fileName, type):
    # Check if the file name follows the pattern "{jornada_value}_{i+1}_footballDay.json"
...
def saveGraph(targetPath, G, fileName):
    ...
    nx.write_gexf(
        G, outputFilePath, version="1.2draft", encoding="utf-8", prettyprint=True
    )


# Manage to generate and save multidigrpahs.
def managmentMultiGraph(dataFrame, fileName, targetFolder, G):
    ...
    validRecipientId = dataFrame.loc[dataFrame["pass_recipient_id"].notna()]
    _ = validRecipientId.apply(addEdgesWithAttributes, args=(G,), axis=1)
    # Verification
    assert len(list(G.edges())) == len(
        validRecipientId
    ), f"number of edges and dataframe' size is not the same"
    # Get new fileName
...
# Manage to generate and save digrpahs.
...
def main():
    ...
    (
        ...
        targetFolderDi,
        targetFolderMultiDi,
    ) = generateDynamicPaths(experimentName)
