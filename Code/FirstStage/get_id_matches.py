...
def filtrar_partidos_por_equipo(ruta_fichero_season_id, nombre_equipo):
    ...
    ruta_season_id = os.path.join(ruta_output, ruta_fichero_season_id)  # Ruta del archivo de la temporada
    ...
    partidos_seleccionados = {}  # Diccionario para almacenar los partidos seleccionados por el equipo
    ...
    for partido in datos:
        ...
        if nombre_equipo.lower() == equipo_local.lower() or nombre_equipo.lower() == equipo_visitante.lower():
            jornada = partido['match_week']  # Obtener el número de jornada del partido
            partido_id = partido['match_id']  # Obtener el ID del partido

            if jornada not in partidos_seleccionados:
                partidos_seleccionados[jornada] = []  # Inicializar la lista de partidos si es la primera vez que se encuentra esa jornada
    ...
    ruta_salida = os.path.join(ruta_output, 'id_matches.json')  # Ruta para guardar el archivo de partidos seleccionados
    
    # Escribir los partidos seleccionados en un archivo JSON
    with open(ruta_salida, 'w') as archivo_salida:
        json.dump(partidos_seleccionados, archivo_salida, indent=4)  # Guardar los datos en el archivo JSON con formato
